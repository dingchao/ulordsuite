// Copyright (c) 2014-2016 The ulordsuite developers
...
import (
	...
	"github.com/ulordsuite/ulordutil/base58"
)
...
func (k *ExtendedKey) pubKeyBytes() []byte {
	...
	// key if needed.
	if len(k.pubKey) == 0 {
		pkx, pky := ulordec.S256().ScalarBaseMult(k.key)
		pubKey := ulordec.PublicKey{Curve: ulordec.S256(), X: pkx, Y: pky}
...
func (k *ExtendedKey) Child(i uint32) (*ExtendedKey, error) {
	...
	if ilNum.Cmp(ulordec.S256().N) >= 0 || ilNum.Sign() == 0 {
	...
	if k.isPrivate {
		...
		ilNum.Mod(ilNum, ulordec.S256().N)
		...
		ilx, ily := ulordec.S256().ScalarBaseMult(il)
		...
		// public key.
		pubKey, err := ulordec.ParsePubKey(k.key, ulordec.S256())
		if err != nil {
			return nil, err
		}
		...
		//
		// childKey = serP(point(parse256(Il)) + parentKey)
		childX, childY := ulordec.S256().Add(ilx, ily, pubKey.X, pubKey.Y)
		pk := ulordec.PublicKey{Curve: ulordec.S256(), X: childX, Y: childY}
		childKey = pk.SerializeCompressed()
	}

	// The fingerprint of the parent for the derived child is the first 4
	// bytes of the RIPEMD160(SHA256(parentPubKey)).
	parentFP := ulordutil.Hash160(k.pubKeyBytes())[:4]
...
// ECPubKey converts the extended key to a ulordec public key and returns it.
func (k *ExtendedKey) ECPubKey() (*ulordec.PublicKey, error) {
	return ulordec.ParsePubKey(k.pubKeyBytes(), ulordec.S256())
}

// ECPrivKey converts the extended key to a ulordec private key and returns it.
...
func (k *ExtendedKey) ECPrivKey() (*ulordec.PrivateKey, error) {
	if !k.isPrivate {
		return nil, ErrNotPrivExtKey
	}

	privKey, _ := ulordec.PrivKeyFromBytes(ulordec.S256(), k.key)
	return privKey, nil
}

// Address converts the extended key to a standard bitcoin pay-to-pubkey-hash
// address for the passed network.
func (k *ExtendedKey) Address(net *chaincfg.Params) (*ulordutil.AddressPubKeyHash, error) {
	pkHash := ulordutil.Hash160(k.pubKeyBytes())
	return ulordutil.NewAddressPubKeyHash(pkHash, net)
}
...
func NewMaster(seed []byte, net *chaincfg.Params) (*ExtendedKey, error) {
	...
	if secretKeyNum.Cmp(ulordec.S256().N) >= 0 || secretKeyNum.Sign() == 0 {
...
func NewKeyFromString(key string) (*ExtendedKey, error) {
	...
	if isPrivate {
		...
		if keyNum.Cmp(ulordec.S256().N) >= 0 || keyNum.Sign() == 0 {
			return nil, ErrUnusableSeed
		}
	} else {
		// Ensure the public key parses correctly and is actually on the
		// secp256k1 curve.
		_, err := ulordec.ParsePubKey(keyData, ulordec.S256())
		if err != nil {
			return nil, err
		}
	}
