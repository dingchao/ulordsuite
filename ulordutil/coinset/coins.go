...
func (cs *CoinSet) TotalValue() (value ulordutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue ulordutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue ulordutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount ulordutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue ulordutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue ulordutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue ulordutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue ulordutil.Amount, coins []Coin) (Coins, error) {
...
// ulordutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() ulordutil.Amount {
	return ulordutil.Amount(c.txOut().Value)
}
