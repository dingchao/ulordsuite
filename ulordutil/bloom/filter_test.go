...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := ulordutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(ulordutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := ulordutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := ulordutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
