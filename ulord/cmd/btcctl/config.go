...
const (
	...
	unusableFlags = ulordjson.UFWebsocketOnly | ulordjson.UFNotification
)

var (
	ulordHomeDir           = ulordutil.AppDataDir("ulord", false)
	btcctlHomeDir         = ulordutil.AppDataDir("btcctl", false)
	btcwalletHomeDir      = ulordutil.AppDataDir("btcwallet", false)
	defaultConfigFile     = filepath.Join(btcctlHomeDir, "btcctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(ulordHomeDir, "rpc.cert")
...
func listCommands() {
	...
	cmdMethods := ulordjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := ulordjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := ulordjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&ulordjson.UFWalletOnly != 0 {
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		...
		if preCfg.Wallet {
			...
			serverConfigPath = filepath.Join(ulordHomeDir, "ulord.conf")
		}
...
// For this it tries to read the config file for the RPC server (either ulord or
