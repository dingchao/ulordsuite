...
func main() {
	...
	method := args[0]
	usageFlags, err := ulordjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := ulordjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// ulordjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(ulordjson.Error); ok {
		...
		// The error is not a ulordjson.Error and this really should not
	...
	marshalledJSON, err := ulordjson.MarshalCmd(1, cmd)
	if err != nil {
