...
import (
	...
	"github.com/ulordsuite/winsvc/eventlog"
	"github.com/ulordsuite/winsvc/mgr"
	"github.com/ulordsuite/winsvc/svc"
)

const (
	// svcName is the name of ulord service.
	svcName = "ulordsvc"
...
// logServiceStartOfDay logs information about ulord when the main server has
...
// ulordService houses the main service handler which handles all service
// updates and launching ulordMain.
type ulordService struct{}
...
// long-running ulordMain (which is the real meat of ulord), handles service
// change requests, and notifies the service control manager of changes.
func (s *ulordService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start ulordMain in a separate goroutine so the service can start
...
// installService attempts to install the ulord service.  Typically this should
...
// removeService attempts to uninstall the ulord service.  Typically this should
...
// startService attempts to start the ulord service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &ulordService{})
	if err != nil {
