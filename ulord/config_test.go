...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-ulord.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "ulord")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of ulord binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-ulord.conf")
