...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: ulordutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := ulordec.PrivKeyFromBytes(ulordec.S256(), []byte{0x01})
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := ulordutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*ulordutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, ulordutil.NewTx(tx))
	}
...
func additionalCoinbase(amount ulordutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee ulordutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee ulordutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee ulordutil.Amount) *wire.MsgTx {
