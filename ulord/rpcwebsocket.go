// Copyright (c) 2013-2017 The ulordsuite developers
...
import (
	...
	"github.com/ulordsuite/ulord/database"
...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *ulordutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *ulordutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *ulordutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a ulordutil.Address) {
	switch a := a.(type) {
	case *ulordutil.AddressPubKeyHash:
		f.pubKeyHashes[*a.Hash160()] = struct{}{}
		return
	case *ulordutil.AddressScriptHash:
		f.scriptHashes[*a.Hash160()] = struct{}{}
		return
	case *ulordutil.AddressPubKey:
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := ulordutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a ulordutil.Address) bool {
	switch a := a.(type) {
	case *ulordutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a ulordutil.Address) {
	switch a := a.(type) {
	case *ulordutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected ulordutil.Block
type notificationBlockDisconnected ulordutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *ulordutil.Tx,
...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := ulordjson.NewBlockConnectedNtfn(block.Hash().String(), block.Height(),
		block.MsgBlock().Header.Timestamp.Unix())
	marshalledJSON, err := ulordjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *ulordutil.Block) {
	...
	ntfn := ulordjson.NewBlockDisconnectedNtfn(block.Hash().String(),
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := ulordjson.NewFilteredBlockConnectedNtfn(block.Height(),
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := ulordjson.NewFilteredBlockDisconnectedNtfn(block.Height(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *ulordutil.Tx) {
	...
	ntfn := ulordjson.NewTxAcceptedNtfn(txHashStr, ulordutil.Amount(amount).ToBTC())
	marshalledJSON, err := ulordjson.MarshalCmd(nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *ulordjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = ulordjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = ulordjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*ulordutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *ulordutil.Block, txIndex int) *ulordjson.BlockDetails {
	if block == nil {
		return nil
	}
	return &ulordjson.BlockDetails{
		Height: block.Height(),
...
func newRedeemingTxNotification(txHex string, index int, block *ulordutil.Block) ([]byte, error) {
	// Create and marshal the notification.
	ntfn := ulordjson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return ulordjson.MarshalCmd(nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *ulordutil.Tx, block *ulordutil.Block) {
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			ntfn := ulordjson.NewRecvTxNtfn(txHex, blockDetails(block,
				tx.Index()))
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *ulordutil.Tx,
	clients map[chan struct{}]*wsClient) {

	clientsToNotify := m.subscribedClients(tx, clients)

	if len(clientsToNotify) != 0 {
		n := ulordjson.NewRelevantTxAcceptedNtfn(txHexString(tx.MsgTx()))
		marshalled, err := ulordjson.MarshalCmd(nil, n)
		if err != nil {
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *ulordutil.Tx, block *ulordutil.Block) {
...
out:
	for {
		...
		if err != nil {
			...
			jsonErr := &ulordjson.RPCError{
				Code:    ulordjson.ErrRPCParse.Code,
		...
		switch authCmd, ok := cmd.cmd.(*ulordjson.AuthenticateCmd); {
		...
		if !c.isAdmin {
			if _, ok := rpcLimited[request.Method]; !ok {
				jsonErr := &ulordjson.RPCError{
					Code:    ulordjson.ErrRPCInvalidParams.Code,
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*ulordjson.HelpCmd)
	if !ok {
		return nil, ulordjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*ulordjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &ulordjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*ulordjson.NotifySpentCmd)
	if !ok {
		return nil, ulordjson.ErrRPCInternal
	}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*ulordjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, ulordjson.ErrRPCInternal
	}
...
func handleNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*ulordjson.NotifyReceivedCmd)
	if !ok {
		return nil, ulordjson.ErrRPCInternal
	}
...
func handleStopNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*ulordjson.StopNotifySpentCmd)
	if !ok {
		return nil, ulordjson.ErrRPCInternal
	}
...
func handleStopNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*ulordjson.StopNotifyReceivedCmd)
	if !ok {
		return nil, ulordjson.ErrRPCInternal
	}
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := ulordutil.DecodeAddress(addr, params)
		if err != nil {
			return &ulordjson.RPCError{
				Code: ulordjson.ErrRPCInvalidAddressOrKey,
...
func deserializeOutpoints(serializedOuts []ulordjson.OutPoint) ([]*wire.OutPoint, error) {
...
var ErrRescanReorg = ulordjson.RPCError{
	Code:    ulordjson.ErrRPCDatabase,
	Message: "Reorganize",
}

// rescanBlock rescans all transactions in a single block.  This is a helper
// function for handleRescan.
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *ulordutil.Block) {
	for _, tx := range blk.Transactions() {
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				ntfn := ulordjson.NewRecvTxNtfn(txHex,
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *ulordutil.Block, params *chaincfg.Params) []string {
...
func handleRescanBlocks(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*ulordjson.RescanBlocksCmd)
	if !ok {
		return nil, ulordjson.ErrRPCInternal
	}
	...
	discoveredData := make([]ulordjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, ulordjson.RescannedBlock{
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *ulordutil.Block) error {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*ulordjson.RescanCmd)
	if !ok {
		return nil, ulordjson.ErrRPCInternal
	}
	...
	for _, addrStr := range cmd.Addresses {
		addr, err := ulordutil.DecodeAddress(addrStr, params)
		if err != nil {
			jsonErr := ulordjson.RPCError{
				Code: ulordjson.ErrRPCInvalidAddressOrKey,
...
fetchRange:
	...
	loopHashList:
		for i := range hashList {
			...
			n := ulordjson.NewRescanProgressNtfn(hashList[i].String(),
				blk.Height(), blk.MsgBlock().Header.Timestamp.Unix())
			mn, err := ulordjson.MarshalCmd(nil, n)
			if err != nil {
	...
	// Notify websocket client of the finished rescan.  Due to how ulord
	...
	// been sent.
	n := ulordjson.NewRescanFinishedNtfn(lastBlockHash.String(),
		lastBlock.Height(),
		lastBlock.MsgBlock().Header.Timestamp.Unix())
	if mn, err := ulordjson.MarshalCmd(nil, n); err != nil {
