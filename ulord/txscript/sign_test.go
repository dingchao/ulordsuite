...
type addressToKey struct {
	key        *ulordec.PrivateKey
	compressed bool
}

func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr ulordutil.Address) (*ulordec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr ulordutil.Address) ([]byte, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key, err := ulordec.NewPrivateKey(ulordec.S256())
			if err != nil {
			...
			pk := (*ulordec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := ulordutil.NewAddressPubKeyHash(
				ulordutil.Hash160(pk), &chaincfg.TestNet3Params)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*ulordec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := ulordutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := ulordutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key1, err := ulordec.NewPrivateKey(ulordec.S256())
			if err != nil {
				t.Errorf("failed to make privKey for %s: %v",
					msg, err)
				break
			...
			pk1 := (*ulordec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := ulordutil.NewAddressPubKey(pk1,
				&chaincfg.TestNet3Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			key2, err := ulordec.NewPrivateKey(ulordec.S256())
			if err != nil {
			...
			pk2 := (*ulordec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := ulordutil.NewAddressPubKey(pk2,
				&chaincfg.TestNet3Params)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*ulordutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
...
func TestSignatureScript(t *testing.T) {
	...
	privKey, _ := ulordec.PrivKeyFromBytes(ulordec.S256(), privKeyD)
