...
func opcodeCheckSig(op *parsedOpcode, vm *Engine) error {
	...
	pubKey, err := ulordec.ParsePubKey(pkBytes, ulordec.S256())
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
	}

	var signature *ulordec.Signature
	if vm.hasFlag(ScriptVerifyStrictEncoding) ||
		vm.hasFlag(ScriptVerifyDERSignatures) {

		signature, err = ulordec.ParseDERSignature(sigBytes, ulordec.S256())
	} else {
		signature, err = ulordec.ParseSignature(sigBytes, ulordec.S256())
	}
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
...
type parsedSigInfo struct {
	signature       []byte
	parsedSignature *ulordec.Signature
	parsed          bool
}
...
func opcodeCheckMultiSig(op *parsedOpcode, vm *Engine) error {
	...
	for numSignatures > 0 {
		...
		var parsedSig *ulordec.Signature
		if !sigInfo.parsed {
			if err := vm.checkHashTypeEncoding(hashType); err != nil {
				return err
			}
			...
			if vm.hasFlag(ScriptVerifyStrictEncoding) ||
				...
				parsedSig, err = ulordec.ParseDERSignature(signature,
					ulordec.S256())
			} else {
				parsedSig, err = ulordec.ParseSignature(signature,
					ulordec.S256())
			}
			sigInfo.parsed = true
			if err != nil {
		...
		// Parse the pubkey.
		parsedPubKey, err := ulordec.ParsePubKey(pubKey, ulordec.S256())
		if err != nil {
			continue
		}
