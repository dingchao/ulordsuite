...
func (s *SigCache) Exists(sigHash chainhash.Hash, sig *ulordec.Signature, pubKey *ulordec.PublicKey) bool {
	s.RLock()
...
func (s *SigCache) Add(sigHash chainhash.Hash, sig *ulordec.Signature, pubKey *ulordec.PublicKey) {
	s.Lock()
	defer s.Unlock()
