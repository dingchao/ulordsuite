...
// newAddressPubKey returns a new ulordutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) ulordutil.Address {
	addr, err := ulordutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new ulordutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) ulordutil.Address {
	addr, err := ulordutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new ulordutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) ulordutil.Address {
	addr, err := ulordutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the ulordutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the ulordutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := ulordutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := ulordutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := ulordutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := ulordutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := ulordutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*ulordutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*ulordutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*ulordutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*ulordutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*ulordutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
