...
// test data.
func genRandomSig() (*chainhash.Hash, *ulordec.Signature, *ulordec.PublicKey, error) {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	sig1Copy, _ := ulordec.ParseSignature(sig1.Serialize(), ulordec.S256())
	key1Copy, _ := ulordec.ParsePubKey(key1.SerializeCompressed(), ulordec.S256())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		sigCopy, _ := ulordec.ParseSignature(sig.Serialize(), ulordec.S256())
		keyCopy, _ := ulordec.ParsePubKey(key.SerializeCompressed(), ulordec.S256())
	...
	sigNewCopy, _ := ulordec.ParseSignature(sigNew.Serialize(), ulordec.S256())
	keyNewCopy, _ := ulordec.ParsePubKey(keyNew.SerializeCompressed(), ulordec.S256())
