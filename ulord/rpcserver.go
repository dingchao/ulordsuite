...
var (
	...
	// data.
	gbtCoinbaseAux = &ulordjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &ulordjson.RPCError{
		Code:    ulordjson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &ulordjson.RPCError{
		Code:    ulordjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which ulord has no support because
...
// not needed.
func internalRPCError(errStr, context string) *ulordjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return ulordjson.NewRPCError(ulordjson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *ulordjson.RPCError {
	return ulordjson.NewRPCError(ulordjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *ulordjson.RPCError {
	return ulordjson.NewRPCError(ulordjson.ErrRPCNoTxInfo,
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > ulordutil.MaxSatoshi {
			return nil, &ulordjson.RPCError{
				Code:    ulordjson.ErrRPCType,
				Message: "Invalid amount",
			}
		}

		// Decode the provided address.
		addr, err := ulordutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &ulordjson.RPCError{
		...
		satoshi, err := ulordutil.NewAmount(amount)
		if err != nil {
...
// transaction.
func createVinList(mtx *wire.MsgTx) []ulordjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]ulordjson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &ulordjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []ulordjson.Vout {
	voutList := make([]ulordjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout ulordjson.Vout
		vout.N = uint32(i)
		vout.Value = ulordutil.Amount(v.Value).ToBTC()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*ulordjson.TxRawResult, error) {
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &ulordjson.RPCError{
			Code:    ulordjson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := ulordjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.DecodeScriptCmd)
	...
	p2sh, err := ulordutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := ulordjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateFee(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.EstimateFeeCmd)
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if len(cfg.miningAddrs) == 0 {
		return nil, &ulordjson.RPCError{
			Code: ulordjson.ErrRPCInternal.Code,
	...
	if !s.cfg.ChainParams.GenerateSupported {
		return nil, &ulordjson.RPCError{
			Code: ulordjson.ErrRPCDifficulty,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &ulordjson.RPCError{
				Code:    ulordjson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*ulordjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result ulordjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = ulordjson.Bool(peer.Connected())
		...
		addrs := make([]ulordjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr ulordjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &ulordjson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := ulordutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	blockReply := ulordjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]ulordjson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &ulordjson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		Bip9SoftForks: make(map[string]*ulordjson.Bip9SoftForkDescription),
	}
	...
	chainInfo.SoftForks = []*ulordjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.Bip9SoftForks[forkName] = &ulordjson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &ulordjson.RPCError{
			Code:    ulordjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := ulordjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
// state as a ulordjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*ulordjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]ulordjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := ulordutil.NewTx(tx)
		resultTx := ulordjson.GetBlockTemplateResultTx{
	...
	reply := ulordjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *ulordjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &ulordjson.RPCError{
			Code:    ulordjson.ErrRPCClientNotConnected,
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &ulordjson.RPCError{
			Code:    ulordjson.ErrRPCClientInInitialDownload,
...
func handleGetBlockTemplateProposal(s *rpcServer, request *ulordjson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &ulordjson.RPCError{
			Code: ulordjson.ErrRPCType,
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.cfg.CfIndex == nil {
		return nil, &ulordjson.RPCError{
			Code:    ulordjson.ErrRPCNoCFIndex,
			Message: "The CF index must be enabled for this command",
		}
	}

	c := cmd.(*ulordjson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*ulordjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := ulordjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := ulordjson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &ulordjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*ulordjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*ulordjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &ulordjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &ulordjson.RPCError{
				Code: ulordjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &ulordjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         ulordutil.Amount(value).ToBTC(),
		ScriptPubKey: ulordjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]ulordjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]ulordjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]ulordjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &ulordjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     ulordutil.Amount(originTxOut.Value).ToBTC(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr ulordutil.Address, numToSkip, numRequested uint32) ([]*ulordutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*ulordjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := ulordutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &ulordjson.RPCError{
			Code:    ulordjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]ulordjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.SendRawTransactionCmd)
...
func handleSetGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.SetGenerateCmd)
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.ValidateAddressCmd)

	result := ulordjson.ValidateAddressChainResult{}
	addr, err := ulordutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyChain(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.VerifyChainCmd)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*ulordjson.VerifyMessageCmd)
	...
	if _, ok := addr.(*ulordutil.AddressPubKeyHash); !ok {
		return nil, &ulordjson.RPCError{
			Code:    ulordjson.ErrRPCType,
	...
	pk, wasCompressed, err := ulordec.RecoverCompact(ulordec.S256(), sig,
		expectedMessageHash)
	if err != nil {
	...
	address, err := ulordutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
//
// NOTE: This is a ulordsuite extension ported from github.com/decred/dcrd.
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]ulordjson.VersionResult{
		"ulordjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, ulordjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *ulordjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(ulordjson.Error); ok &&
			jerr.ErrorCode == ulordjson.ErrUnregisteredMethod {

			parsedCmd.err = ulordjson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = ulordjson.NewRPCError(
			ulordjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *ulordjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *ulordjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*ulordjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return ulordjson.MarshalResponse(id, result, jsonErr)
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="ulord RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *ulordutil.Block, flags blockchain.BehaviorFlags) (bool, error)
