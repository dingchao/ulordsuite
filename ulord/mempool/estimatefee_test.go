...
func (eft *estimateFeeTester) testTx(fee ulordutil.Amount) *TxDesc {
	eft.version++
	return &TxDesc{
		TxDesc: mining.TxDesc{
			Tx: ulordutil.NewTx(&wire.MsgTx{
				Version: eft.version,
			}),
			Height: eft.height,
			Fee:    int64(fee),
...
func (eft *estimateFeeTester) newBlock(txs []*wire.MsgTx) {
	...
	block := ulordutil.NewBlock(&wire.MsgBlock{
		Transactions: txs,
	})
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	...
	for i := uint32(0); i < txPerRound; i++ {
		newTx := eft.testTx(ulordutil.Amount(rand.Intn(1000000)))
