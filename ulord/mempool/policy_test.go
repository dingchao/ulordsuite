...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee ulordutil.Amount // minimum relay transaction fee.
...
func TestCheckPkScriptStandard(t *testing.T) {
	var pubKeys [][]byte
	for i := 0; i < 4; i++ {
		pk, err := ulordec.NewPrivateKey(ulordec.S256())
		if err != nil {
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: ulordutil.MaxSatoshi, PkScript: pkScript},
			ulordutil.MaxSatoshi,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := ulordutil.NewAddressPubKeyHash(addrHash[:],
	...
	for _, test := range tests {
		...
		err := checkTransactionStandard(ulordutil.NewTx(&test.tx),
