...
ulord provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, ulord supports a hybrid,
...
Configuring ulord as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the ulord command
line or in the ulord configuration file.  Typically the Tor proxy address will be
...
By default, ulord assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since ulord will act as the server.
...
```text
HiddenServiceDir /var/tor/ulord
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring ulord as a Tor hidden service requires
three flags:
...
```bash
$ ./ulord --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
ulord provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the ulord command line or in the ulord
...
```bash
$ ./ulord --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
ulord provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./ulord --proxy=127.0.0.1:9050 --torisolation
```
