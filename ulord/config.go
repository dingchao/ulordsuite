...
import (
	...
	"github.com/ulordsuite/ulord/connmgr"
	"github.com/ulordsuite/ulord/database"
	_ "github.com/ulordsuite/ulord/database/ffldb"
...
const (
	defaultConfigFilename        = "ulord.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "ulord.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-ulord.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = ulordutil.AppDataDir("ulord", false)
...
// config defines the configuration options for ulord.
//
...
//
// The above results in ulord functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		ulordLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = ulordutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]ulordutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := ulordutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		ulordLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-ulord.conf to the given destination path,
...
// ulordDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func ulordDial(addr net.Addr) (net.Conn, error) {
...
// ulordLookup resolves the IP of the given host using the correct DNS lookup
...
func ulordLookup(host string) ([]net.IP, error) {
