...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   ulordjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{ulordjson.UFWalletOnly, "UFWalletOnly"},
		{ulordjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{ulordjson.UFNotification, "UFNotification"},
		{ulordjson.UFWalletOnly | ulordjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{ulordjson.UFWalletOnly | ulordjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := ulordjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: ulordjson.Error{ErrorCode: ulordjson.ErrDuplicateMethod},
		},
		{
			...
			flags: ulordjson.TstHighestUsageFlagBit,
			err:   ulordjson.Error{ErrorCode: ulordjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: ulordjson.Error{ErrorCode: ulordjson.ErrEmbeddedType},
		},
		{
			...
			err: ulordjson.Error{ErrorCode: ulordjson.ErrUnexportedField},
		},
		{
			...
			err: ulordjson.Error{ErrorCode: ulordjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: ulordjson.Error{ErrorCode: ulordjson.ErrNonOptionalField},
		},
		{
			...
			err: ulordjson.Error{ErrorCode: ulordjson.ErrNonOptionalDefault},
		},
		{
			...
			err: ulordjson.Error{ErrorCode: ulordjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := ulordjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestRegisteredCmdMethods(t *testing.T) {
	...
	methods := ulordjson.RegisteredCmdMethods()
	if len(methods) == 0 {
