...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if ulordjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *ulordjson.RPCError {
				return ulordjson.NewRPCError(ulordjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := ulordjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := ulordjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := ulordjson.Error{ErrorCode: ulordjson.ErrInvalidType}
	_, err = ulordjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(ulordjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = ulordjson.MarshalResponse(1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		in   *ulordjson.RPCError
		want string
	}{
		{
			ulordjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			ulordjson.ErrRPCMethodNotFound,
