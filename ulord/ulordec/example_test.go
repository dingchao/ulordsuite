...
func Example_signMessage() {
	...
	privKey, pubKey := ulordec.PrivKeyFromBytes(ulordec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := ulordec.ParsePubKey(pubKeyBytes, ulordec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := ulordec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := ulordec.PrivKeyFromBytes(ulordec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := ulordec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
