// Copyright (c) 2013-2014 The ulordsuite developers
...
// oldBtcdHomeDir returns the OS specific home directory ulord used prior to
// version 0.3.3.  This has since been replaced with ulordutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldBtcdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "ulord")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".ulord")
	}
...
// location prior to ulord version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to ulord
...
func upgradeDBPaths() error {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "ulord.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "ulord_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "ulord_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to ulord
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		ulordLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old ulord.conf into new location if needed.
		...
		if ohpEmpty {
			...
			ulordLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to ulord as new versions require it.
