...
import (
	...
	"github.com/ulordsuite/ulord/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt ulordutil.Amount) (*ulordec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := ulordutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	ulordCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, ulordCfg)
	if err != nil {
	...
	const outputValue = ulordutil.SatoshiPerBitcoin
	...
	txns := []*ulordutil.Tx{ulordutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis ulordutil.Amount, timeLock int32,
	...
	p2shAddr, err := ulordutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = ulordutil.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*ulordutil.Tx{ulordutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*ulordutil.Tx{ulordutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
