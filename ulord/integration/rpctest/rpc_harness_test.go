...
func testSendOutputs(r *Harness, t *testing.T) {
	...
	// input.
	txid := genSpend(ulordutil.Amount(5 * ulordutil.SatoshiPerBitcoin))
	...
	txid = genSpend(ulordutil.Amount(500 * ulordutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := ulordutil.Amount(50 * ulordutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := ulordutil.Amount(numMatureOutputs * 50 * ulordutil.SatoshiPerBitcoin)
