...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	ulordPath, err := ulordExecutablePath()
	if err != nil {
		ulordPath = "ulord"
	}
...
// arguments returns an array of arguments that be used to launch the ulord
// process.
...
// command returns the exec.Cmd which will be used to start the ulord process.
...
// to the ulord process that is launched via Start().
...
// stop interrupts the running ulord process process, and waits until it exits
...
// shutdown terminates the running ulord process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := ulordutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
