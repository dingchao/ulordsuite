...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *ulordjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *ulordjson.NotifyNewTransactionsCmd:
...
// returning an unmarshaled ulordjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]ulordjson.OutPoint, 0, nslen)
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := ulordjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := ulordjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
