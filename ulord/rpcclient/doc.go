...
Bitcoin RPC server that uses a ulord/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with ulord (https://github.com/ulordsuite/ulord),
btcwallet (https://github.com/ulordsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, ulord and
...
ulord or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by ulord and
...
All notifications provided by ulord require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by ulord (and
...
Also, it is important to realize that ulord intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the ulord RPC server directly, only the RPCs which are related to
...
detected by type asserting the error in a *ulordjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*ulordjson.RPCError); ok {
  		switch jerr.Code {
  		case ulordjson.ErrRPCUnimplemented:
